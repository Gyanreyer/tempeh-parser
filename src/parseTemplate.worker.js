...
/**
 * @typedef {{
 *  writableStream: WritableStream<TmphNode>;
 * } & ({
 *  filePath: string;
 *  rawHTMLString?: never;
 * } | {
 ...
 */
export default function parseTemplate({ writableStream, ...lexerParams }) {
  /**
          ...
          switch (token.type) {
            ...
            case LexerTokenType.TEXT_CONTENT: {
              ...
              if (currentOpenLeafElementNode) {
                const previousNode =
                  currentOpenLeafElementNode.children?.at(-1);
            ...
            case LexerTokenType.OPENING_TAGNAME: {
              ...
              if (!currentOpenLeafElementNode) {
                controller.error(
                  new Error(
                    `Tempeh parsing error: Encountered unexpected opening tag "${
                      token.value
                    }" at ${
                      lexerParams.filePath ? `${lexerParams.filePath}:` : ""
                    }${token.l}:${token.c}`
            ...
            case LexerTokenType.CLOSING_TAGNAME: {
              ...
              if (!currentOpenLeafElementNode) {
                controller.error(
                  new Error(
                    `Tempeh parsing error: Encountered unexpected closing tag "${closedTagname}" at ${
                      lexerParams.filePath ? `${lexerParams.filePath}:` : ""
                    }${token.l}:${token.c}`
                  )
                );
